...
<div class="sf_admin_form">
    ...
    <?php foreach ($configuration->getFormFields($form, 'show') as $fieldset => $fields): ?>
      <?php //include_partial('AdministrarClientes/form_fieldset', array('cliente' => $cliente, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset)) ?>
		...
		<div id="sf_fieldset_<?php echo preg_replace('/[^a-z0-9_]/', '_', strtolower($fieldset)) ?>">
			...
			<?php if ($field->isPartial()): ?>
		      <?php include_partial('AdministrarClientes/'.$name, array('cliente' => $cliente, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php elseif ($field->isComponent()): ?>
		      <?php include_component('AdministrarClientes', $name, array('cliente' => $cliente, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php else: ?>
