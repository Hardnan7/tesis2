      ...
      <?php //include_partial('AdministrarEquipos/form_fieldset', array('equipo' => $equipo, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset)) ?>
		...
		<div id="sf_fieldset_<?php echo preg_replace('/[^a-z0-9_]/', '_', strtolower($fieldset)) ?>">
			...
			<?php if ($field->isPartial()): ?>
		      <?php include_partial('AdministrarEquipos/'.$name, array('equipo' => $equipo, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php elseif ($field->isComponent()): ?>
		      <?php include_component('AdministrarEquipos', $name, array('equipo' => $equipo, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php else: ?>
